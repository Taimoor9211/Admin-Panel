...
import ProviderRegistrationCard from "./ProviderRegistrationCard";

const JobCardsGrid = ({ activeTab, jobCards, setShowPasswordModal }) => {
  const isAccountOrProviderTab = activeTab === "Account Change" || activeTab === "Provider Registration";
  ...
  return (
    <>
      {isAccountOrProviderTab ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6 lg:pr-40">
          {jobCards.map((job) => (
            activeTab === "Account Change" ? (
              ...
              <ProviderRegistrationCard key={job.id} job={job} />
            )
          ))}
        </div>
      ) : (
